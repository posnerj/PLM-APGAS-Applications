...
public class BCG {
  ...
  private static Double[] compute(String[] args) throws ParseException {
    ...
    options.addOption("g", true, "Number of nodes to process before probing. Default 200.");
    ...
    options.addOption("ss", true, "split for SplitQueue, range is between 0 and 10, default ist 5");
    options.addOption(
    ...
    double split = Double.parseDouble(cmd.getOptionValue("ss", "5")) / 10; // paper
    double steal = Double.parseDouble(cmd.getOptionValue("f", "5")) / 10; // paper
    ...
    SerializableCallable<Queue> init =
        () -> new Queue(new Rmat(seed, n, a, b, c, d), permute, split, steal);

    GLBParameters glbPara = new GLBParameters(g, w, l, z, m, verbose, timestamps, numPlaces);
    GLBCoord<Queue, Double> glb = new GLBCoord<Queue, Double>(init, glbPara, false);
