...
public class IncFTTimedGLB<Queue extends IncFTTaskQueue<Queue, T>, T extends Serializable>
  ...
  IncFTTimedGLBParameters glbPara;
  IncFTTimedWorker<Queue, T> worker;
  private int p = places().size();

  public IncFTTimedGLB(
      SerializableCallable<Queue> init, IncFTTimedGLBParameters glbPara, boolean tree) {
    ...
    for (int i = 0; i < places().size(); i++) {
      ...
      while (this.placeKeyMap.size() < places().size()) {
        try {
          ...
          if (null == partition) {
            System.err.println(
                "IncFTTimedGLB constructor hz.getPartitionService().getPartition("
                    + key
                    + ") is null!!!!");
          }
    ...
    SerializableCallable<IncFTTimedWorker<Queue, T>> workerInit =
        () ->
            new IncFTTimedWorker<Queue, T>(
                init,
                glbPara.n,
                glbPara.w,
                glbPara.l,
                ...
                glbPara.P,
                this.placeKeyMap);
    ConsolePrinter.getInstance().println("IncFTTimedGLB constructor five");

    ConsolePrinter.getInstance().println(placeKeyMap.toString());
    this.worker = PlaceLocalObject.make(places(), workerInit);
    ConsolePrinter.getInstance().println("IncFTTimedGLB constructor six");
  ...
  public T[] runParallel() {
    crunchNumberTime = System.nanoTime();
    IncFTTimedWorker.broadcast(worker);
    long now = System.nanoTime();
    crunchNumberTime = now - crunchNumberTime;
    T[] r = collectResults();
    end(r);
  ...
  private void end(T[] r) {
    if (0 != (glbPara.v & IncFTTimedGLBParameters.SHOW_RESULT_FLAG)) {
      rootGlbR.display(r);
    }

    if (0 != (glbPara.v & IncFTTimedGLBParameters.SHOW_TIMING_FLAG)) {
    ...
    if (0 != (glbPara.v & IncFTTimedGLBParameters.SHOW_TASKFRAME_LOG_FLAG)) {
      //      printLog();
    }

    if (0 != (glbPara.v & IncFTTimedGLBParameters.SHOW_GLB_FLAG)) {
      long now = System.nanoTime();
