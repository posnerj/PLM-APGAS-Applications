...
public final class Worker<Queue extends TaskQueue<Queue, T>, T extends Serializable>
  ...
  static <Queue extends TaskQueue<Queue, T>, T extends Serializable> void broadcast(
      GlobalRef<Worker<Queue, T>> globalRef) {
    ...
    if (thieves.getSize() > 0) {
      ...
      if (thief >= 0) {
        ...
        uncountedAsyncAt(
            ...
            () -> {
              //                    int newId =
              // globalRef.get().logger.startStoppingTime(Logger.STEALING);
  ...
  /**
   * Distribute works to (lifeline) thieves by calling the {@link #give(GlobalRef, TaskBag)}
   *
   * @param globalRef GlobalRef of FTWorker
   */
  public void distribute(GlobalRef<Worker<Queue, T>> globalRef) {
   ...
   * @param globalRef GlobalRef of FTWorker
   */
  public void reject(GlobalRef<Worker<Queue, T>> globalRef) {
      ...
      if (thief >= 0) {
        ...
        uncountedAsyncAt(
            ...
            () -> {
              ...
              synchronized (globalRef.get().waiting) {
                int newId = globalRef.get().logger.startStoppingTime(Logger.COMMUNICATION);
                globalRef
                    .get()
                    .consolePrinter
   ...
   * when victim has no workload to share or (ii) {@link #give(GlobalRef, TaskBag)}
   *
   * @param globalRef GlobalRef of FTWorker
   * @return !empty.get();
   */
    ...
    for (int i = 0; i < w && empty.get(); ++i) {
      ...
      try {
        uncountedAsyncAt(
            ...
            () -> {
              //                int newId =
              // globalRef.get().logger.startStoppingTime(Logger.DISTRIBUTING);
   ...
   */
  public void request(GlobalRef<Worker<Queue, T>> globalRef, int thief, boolean lifeline) {
      ...
      if (empty.get() || waiting.get()) {
        ...
        uncountedAsyncAt(
            ...
            () -> {
              ...
              synchronized (globalRef.get().waiting) {
                int thiefNewId = globalRef.get().logger.startStoppingTime(Logger.COMMUNICATION);
                globalRef
                    .get()
                    .consolePrinter
                ...
                globalRef.get().waiting.set(false);
                globalRef.get().waiting.notifyAll();
                globalRef.get().logger.endStoppingTime(thiefNewId);
              }
            });
      } else {
        consolePrinter.println(
   ...
   */
  public void processStack(GlobalRef<Worker<Queue, T>> globalRef) {
   ...
   */
  public void main(GlobalRef<Worker<Queue, T>> ref, Runnable start) {
   ...
   *     {@link TaskQueue} constructor.
   */
  public void main(GlobalRef<Worker<Queue, T>> ref) {
