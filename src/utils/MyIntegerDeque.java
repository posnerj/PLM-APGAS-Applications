 ...
 * @since 1.6
 */
public class MyIntegerDeque implements Cloneable, Serializable {
   ...
   */
  transient int[] elements; // non-private to simplify nested class access
  /**
  ...
  public MyIntegerDeque() {
    elements = new int[MIN_INITIAL_CAPACITY * 2];
  }
   ...
   */
  public MyIntegerDeque(int numElements) {
   ...
   * @since 1.8
   */
  public int[] peekFromFirst(int n) {
  ...
  public int[] getFromFirst(int n) {
    ...
    int[] result = this.peekFromFirst(toCopy);
    if (toCopy == this.size()) {
      //            this.size = 0;
      this.head = 0;
      this.tail = 0;
  ...
  public int[] getFromLast(int n) {
    ...
    int[] result = this.peekFromLast(toCopy);
    if (toCopy == this.size()) {
      this.head = 0;
      this.tail = 0;
    } else {
  ...
  public void pushArrayLast(int[] elements) {
    this.normalize();
  ...
  public void pushArrayFirst(int[] elements) {
    this.normalize();
    int[] oldElements = this.elements;
    int oldSize = this.size();
    int newSize = oldSize + elements.length;
