...
public class MyArrayDeque<E> extends AbstractCollection<E>
  ...
  public E[] peekFromFirst(int n) {
    return peekFromFirst(n, 0);
  }

  @SuppressWarnings("unchecked")
  public E[] peekFromFirst(int n, int offset) {
    final int toCopy = Math.min(n, this.size() - offset);
    Object[] result = new Object[toCopy];
  ...
  @SuppressWarnings("unchecked")
  public E[] getFromFirst(int n) {
    int toCopy = Math.min(n, this.size());
    if (0 == toCopy) {
      return (E[]) new Object[0];
    }
    Object[] result = this.peekFromFirst(toCopy);
    if (toCopy == this.size()) {
      //            this.size = 0;
      this.head = 0;
      this.tail = 0;
  ...
  public E[] getFromLast(int n) {
    ...
    Object[] result = this.peekFromLast(toCopy);
    if (toCopy == this.size()) {
      this.head = 0;
      this.tail = 0;
    } else {
  ...
  private void normalize() {
    ...
    Object[] newElements = new Object[n];
  ...
  public void pushArrayLast(E[] elements) {
    this.normalize();
  ...
  public void pushArrayFirst(E[] elements) {
    this.normalize();
    Object[] oldElements = this.elements;
    int oldSize = this.size();
    int newSize = oldSize + elements.length;
