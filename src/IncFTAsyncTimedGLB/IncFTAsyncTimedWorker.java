 ...
 * @param <T> Result type.
 */
public final class IncFTAsyncTimedWorker<
        Queue extends IncFTTaskQueue<Queue, T>, T extends Serializable>
  ...
  private boolean takeNewSnap = true;

  private Future<Void> currentBackup;

  /**
   * Class constructor
   *
   * @param init function closure to init the local {@link IncFTAsyncTimedWorker}
  ...
  /**
   * Internal method used by {@link IncFTAsyncTimedGLB} to start FTWorker at each place when the
    ...
    do {
      ...
      do {
        ...
        synchronized (this.waiting) {
          ...
          if (takeNewSnap == true) { // begin of backup interval
            takeSnapshot();
            takeNewSnap = false;
          }
  ...
  private void waitForBackup() {
    while (this.currentBackup != null) {
      try {
        this.currentBackup.get();
        this.currentBackup = null;
      } catch (Exception e) {
        e.printStackTrace();
      }
   ...
   *
   * @param start init method used in {@link IncFTAsyncTimedWorker}, note the workload is not
   *     allocated, because the workload can only be self-generated.
   */
  ...
  private void writeStealBackupThief(long nStolen, int source) {
    ...
    synchronized (this.waiting) {
      ...
      try {
        ...
        waitForBackup();
        this.currentBackup =
            this.exactlyOnceExecutor.submitOnKey(
                this.iMapBackup,
                getBackupKey(here().id),
                new EntryProcessor() {
                  @Override
