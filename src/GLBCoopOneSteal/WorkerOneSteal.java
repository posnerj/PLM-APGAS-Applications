 ...
 * @param <T> Result type.
 */
public final class WorkerOneSteal<Queue extends TaskQueue<Queue, T>, T extends Serializable>
  ...
  public void distribute() {
    synchronized (this.lifelineThieves) {
      if (thieves.size() + lifelineThieves.size() > 0) {
      ...
      TaskBag loot;
      while (((thieves.size() > 0) || (lifelineThieves.size() > 0))
        ...
        if (this.empty.get() || this.waiting.get()) {
          ...
          try {
            uncountedAsyncAt(
                ...
                () -> {
                  synchronized (this.waiting) {
                    int thiefNewId = logger.startStoppingTime(Logger.COMMUNICATION);
                    this.waiting.set(false);
                    this.waiting.notifyAll();
