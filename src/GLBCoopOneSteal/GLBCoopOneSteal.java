...
public class GLBCoopOneSteal<Queue extends TaskQueue<Queue, T>, T extends Serializable>
  ...
  WorkerOneSteal<Queue, T> worker;

  public GLBCoopOneSteal(SerializableCallable<Queue> init, GLBParameters glbPara, boolean tree) {
    ...
    SerializableCallable<WorkerOneSteal<Queue, T>> workerInit =
        () ->
            new WorkerOneSteal<Queue, T>(
                init,
                glbPara.n,
                glbPara.w,
                glbPara.l,
  ...
  public T[] runParallel() {
    ...
    crunchNumberTime = System.nanoTime();
    WorkerOneSteal.broadcast(worker);
    long now = System.nanoTime();
    crunchNumberTime = now - crunchNumberTime;
    consolePrinter.println("[Cooperative.GLBCoop " + here() + "]: reducing result.");
    T[] r = collectResults();
