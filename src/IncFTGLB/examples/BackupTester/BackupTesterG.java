...
public class BackupTesterG {
  ...
  private static Integer[] compute(String[] args) throws ParseException {
    ...
    options.addOption("t", true, "Task count. Default 200.");
    ...
    SerializableCallable<Queue> init = () -> new Queue();
    IncFTGLBParameters glbPara =
        new IncFTGLBParameters(
            n, w, l, z, m, verbose, timestamps, k, crashNumber, backupCount, numPlaces);
    IncFTGLB<Queue, Integer> glb = new IncFTGLB<Queue, Integer>(init, glbPara, true);

    Runnable start = () -> glb.getTaskQueue().init(t);

    Integer[] result = glb.run(start);
    return result;
  }

  public static void main(String[] args) throws ParseException {
    System.out.println(BackupTesterG.class.getName() + " starts");
