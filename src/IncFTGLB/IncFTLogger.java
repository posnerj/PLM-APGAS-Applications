...
public class IncFTLogger implements Serializable {
  ...
  private transient ArrayList<Integer> timeTypes =
  ...
  public IncFTLogger(int timestamps) {
  ...
  public void printStoppedTime() throws IOException {
    if (stoppingResult == null) {
      if (here().id == 0) {
        System.out.println(
            "IncFTGLogger:printStoppedTime(): stoppingTime was not startet, wrong Constructor called?");
      }
      return;
    }
  ...
  public void collect(IncFTLogger logs[]) {
    for (IncFTLogger l : logs) {
      add(l);
    }
  }
  ...
  public void add(IncFTLogger other) {
  ...
  public IncFTLogger get(boolean verbose) {
    if (timestamps > 0) {
  ...
  private void stoppingTimeToResult() {
    if (stoppingTime == null) {
      if (here().id == 0) {
        System.out.println(
            "IncFTGLogger:stoppingTimeToResult(): stoppingTime was not startet, wrong Constructor called?");
      }
      return;
    }
  ...
  private class Time implements Serializable {
    ...
    public Time(long now, int process, boolean automaticStop) {
      this(now, process, automaticStop, IncFTLogger.this.id);
      IncFTLogger.this.id++;
    }
