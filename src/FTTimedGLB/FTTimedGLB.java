...
public class FTTimedGLB<Queue extends FTTaskQueue<Queue, T>, T extends Serializable>
  ...
  public FTTimedGLB(SerializableCallable<Queue> init, FTTimedGLBParameters glbPara, boolean tree) {
    ...
    for (int i = 0; i < places().size(); i++) {
      ...
      while (this.placeKeyMap.size() < places().size()) {
        try {
          ...
          if (null == partition) {
            System.err.println(
                "FTTimedGLB constructor hz.getPartitionService().getPartition("
                    + key
                    + ") is null!!!!");
          }
    ...
    SerializableCallable<FTTimedWorker<Queue, T>> workerInit =
        () ->
            new FTTimedWorker<Queue, T>(
                init,
                glbPara.n,
                glbPara.w,
                glbPara.l,
  ...
  public T[] runParallel() {
    crunchNumberTime = System.nanoTime();
    FTTimedWorker.broadcast(worker);
    long now = System.nanoTime();
    crunchNumberTime = now - crunchNumberTime;
    T[] r = collectResults();
    end(r);
  ...
  private void end(T[] r) {
    if (0 != (glbPara.v & FTTimedGLBParameters.SHOW_RESULT_FLAG)) {
      rootGlbR.display(r);
    }

    if (0 != (glbPara.v & FTTimedGLBParameters.SHOW_TIMING_FLAG)) {
    ...
    if (0 != (glbPara.v & FTTimedGLBParameters.SHOW_TASKFRAME_LOG_FLAG)) {
      //      printLog();
    }

    if (0 != (glbPara.v & FTTimedGLBParameters.SHOW_GLB_FLAG)) {
      long now = System.nanoTime();
