 ...
 * @param <T> Result type.
 */
public final class FTTimedWorker<Queue extends FTTaskQueue<Queue, T>, T extends Serializable>
  ...
  /** Cycle of writing regular backups in seconds */
   ...
   * Class constructor
   *
   * @param init function closure to init the local {@link FTTimedWorker}
  ...
  /**
   * Internal method used by {@link FTGLB} to start FTTimedWorker at each place when the workload is
   * known statically.
   */
  static <Queue extends FTTaskQueue<Queue, T>, T extends Serializable> void broadcast(
      FTTimedWorker<Queue, T> worker) {
  ...
  /**
   * Merge current FTTimedWorker'timestamps taskbag with incoming task bag.
   *
  ...
  /**
   * Main process function of FTTimedWorker. It does 4 things: (1) execute at most n tasks (2)
   * respond to stealing requests (3) when not worth sharing tasks, reject the stealing requests (4)
   * when running out of tasks, steal from others
   */
   ...
   *
   * @param start init method used in {@link FTTimedWorker}, note the workload is not allocated,
